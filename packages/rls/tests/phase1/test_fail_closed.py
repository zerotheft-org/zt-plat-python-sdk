"""
C2 — Fail-Closed Behaviour

Verifies that every invalid/missing auth scenario:
  - Returns HTTP 403 (never 500)
  - Makes zero DB calls
  - Returns a trace_id in the response body for correlation
"""
...
class TestMissingOrMalformedToken:
    ...
    def test_no_auth_header_response_contains_trace_id(self):
        """trace_id must always be present so ops can correlate with logs."""
        app = make_app(make_verifier({}))
        r = TestClient(app, raise_server_exceptions=False).get("/protected")
    ...
    def test_malformed_bearer_prefix_returns_403(self):
        """'Token abc' instead of 'Bearer abc' — wrong scheme."""
        app = make_app(make_verifier({}))
...
class TestJWTVerificationFailures:
    ...
    def test_missing_tenant_claim_returns_403(self):
        """
        JWT is cryptographically valid but has no tenant_id claim.
        This means the Keycloak protocol mapper is not configured correctly.
        """
    ...
    def test_invalid_tenant_uuid_returns_403(self):
        """tenant_id claim present but not a valid UUID string."""
        app = make_app(
            make_verifier({"sub": str(uuid.uuid4()), "tenant_id": "not-a-uuid"})
        )
        r = TestClient(app, raise_server_exceptions=False).get(
            "/protected",
            headers={"Authorization": "Bearer valid.token"},
    ...
    def test_zero_uuid_tenant_returns_403(self):
        """Zero UUID is explicitly rejected by TenantContext.__post_init__."""
        zero_uuid = "00000000-0000-0000-0000-000000000000"
        app = make_app(
            make_verifier({"sub": str(uuid.uuid4()), "tenant_id": zero_uuid})
        )
        r = TestClient(app, raise_server_exceptions=False).get(
            "/protected",
            headers={"Authorization": "Bearer valid.token"},
...
class TestUnexpectedCrash:
    ...
    def test_unexpected_exception_returns_403_never_500(self):
        """
        FAIL-CLOSED: even an unexpected internal crash in the verifier must
        return 403, never 500. A 500 could leak internal stack traces.
        """
